...
object MultipleSchemaSPARQLQueryBuilder {
  ...
  private def processSingleElement
    (g: LinkedGraph, filtersMap: Map[Long, List[String]] = Map.empty, schema: LinkedGraph, query: Query): ElementGroup = {
  ...
  private def processMultipleElements
    (g: LinkedGraph, filtersMap: Map[Long, List[String]] = Map.empty, schema: LinkedGraph, query: Query): ElementGroup = {
  ...
  def apply(tuples: List[(LinkedGraph, Map[Long, List[String]])], schema: LinkedGraph): String = {
    ...
    for((g, filtersMap) <- tuples)
      if(g.getNumberOfNodes == 1){
        body.addElement(
          new ElementOptional(processSingleElement(g, filtersMap, schema, query)))
      }
      else{
        body.addElement(
          new ElementOptional(processMultipleElements(g, filtersMap, schema, query)))
      }
